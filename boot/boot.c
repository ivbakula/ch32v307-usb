...
int main(void)
{
  ...
  if (status) {
    _printf("DVP Device enable failed. Error code: %s\r\n", DVP_Err_String[status]);
    goto _nothing;
  }

  mmem_init();
  /* uint16_t *buff0 = allocm(sizeof(*buff0) * 16); */
  /* uint16_t *buff1 = allocm(sizeof(*buff1) * 16); */
  ...
  if (status) {
    _printf("DVP Device configure failed. Error code: %s\r\n", DVP_Err_String[status]);
    goto _nothing;
  }

  _printf("Initialization successful\r\n");
  _printf("Syclock frequency: %d Hz\r\n", sysclock_frequency);
  _printf("dvp_config.CR0.data: 0x%02x\r\n", dvp_config.CR0.data);
  _printf("dvp_config.CR1.data: 0x%02x\r\n", dvp_config.CR1.data);
  _printf("dvp_config.IFR.data: 0x%02x\r\n", dvp_config.CR1.data);
  _printf("&buff0: 0x%08x\r\n", buff0);
  _printf("&buff1: 0x%08x\r\n", buff1);
